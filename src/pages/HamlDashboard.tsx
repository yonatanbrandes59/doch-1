  ...
  const filtered = useMemo(() => {
    return branchViews
      ...
      .filter((v) =>
        query.trim() === ''
          ? true
          : (v.branch.name + v.branch.region + (v.latest?.coordinatorName ?? ''))
              .toLowerCase()
            ...
            <table className="w-full text-right text-sm">
              <thead>
                <tr className="border-b border-slate-800 text-slate-400">
                  ...
                  <th className="px-3 py-2 font-medium">מחוז</th>
              ...
              <tbody>
                {filtered.map(({ branch, latest, status }) => (
                  <tr key={branch.id} className="border-b border-slate-800/60 hover:bg-slate-800/30">
                    ...
                    <td className="px-3 py-2.5 text-slate-400">{branch.region}</td>
